        ...
        if(Object.keys(localStorage).length === 0){
           document.getElementById("cart_update").disabled = true;
            ...
            document.getElementById("cart_update").addEventListener("click", function() {
                ...
                form.innerHTML = '<div class="underline"></div><form method="post" action="./confirmation.html" id="inscription"><div class="intro"><h2>Vos Coordonnées</h2><p>Attention, délais de livraison modifiés en raison des restrictions COVID19</p><br /></div><div class="shipment"><p><input type="text" name="firstname" id="firstname" placeholder="Prénom" pattern="[a-zA-Z].{3,}" title="4 caractères minimum sans accent" required /><input type="text" name="lastname" id="lastname" placeholder="Nom" pattern="[a-zA-Z].{2,}" title="3 caractères minimum sans accent" required /></p><p><input type="text" name="adresse" id="address" placeholder="adresse" required /></p><p><input type="text" name="cp" id="cp" placeholder="Code Postal." pattern="[0-9].{4,}" title="Format 00000" minlength="4" max="5" required /><input type="text" name="city" id="city" placeholder="Ville." pattern="[a-zA-Z].{3,}" title="3 caractères minimum sans accent" required /></p><p><input type="email" name="email" id="email" placeholder="Adresse e-mail (pour votre facture)" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required/></p></div><input id="validate_order" class="cart_save" form="inscription" type="submit" value="Valider">';
