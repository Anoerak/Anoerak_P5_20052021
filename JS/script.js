        ...
        if(Object.keys(localStorage).length === 0){
           document.getElementById("cart_update").disabled = true;
            ...
            document.getElementById("cart_update").addEventListener("click", function() {
                ...
                form.innerHTML = '<div class="underline"></div><form method="post" action="./confirmation.html" id="inscription"><div class="intro"><h2>Vos Coordonnées</h2><p>Attention, délais de livraison modifiés en raison des restrictions COVID19</p><br /></div><div class="shipment"><p><input type="text" name="firstname" id="firstname" placeholder="Prenom" pattern="^[A-Z]{1}[a-zA-Z\- ]+$" minlength="4" title="4 caractères minimum sans accent" required /><input type="text" name="lastname" id="lastname" placeholder="Nom" pattern="^[A-Z]{1}[a-zA-Z\- ]+$" minlength="3" title="3 caractères minimum sans accent" required /></p><p><input type="text" name="adresse" id="address" pattern="^[0-9]{1,4}[ ,-][ A-Za-zÀ-ÿ0-9\-]+$" placeholder="16 chemin de la..." required /></p><p><input type="text" name="00000" id="cp" placeholder="Code Postal" pattern="^[0-9]{5}$" title="Format 00000" required /><input type="text" name="city" id="city" placeholder="Ville." pattern="[a-zA-Z].{3,}" title="3 caractères minimum sans accent" required /></p><p><input type="email" name="email" id="email" placeholder="Adresse e-mail (pour votre facture)" pattern="^[A-Za-z0-9](([_\\.\\-]?[a-zA-Z0-9]+)*)@([A-Za-z0-9]+)(([_\\.\\-]?[a-zA-Z0-9]+)*)\\.([A-Za-z]{2,})+$" required/></p></div><input id="validate_order" class="cart_save" form="inscription" type="submit" value="Valider">';
    ...
    if((url.substr(-18)) === "/confirmation.html") {
        ...
        document.getElementById('cart_btn').href = "#";
